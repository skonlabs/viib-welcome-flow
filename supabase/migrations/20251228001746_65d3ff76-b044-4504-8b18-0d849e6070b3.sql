-- Fix type mismatch in explain_recommendation
-- The issue is that some calculations return double precision but we need real
...
BEGIN
    ...
    -- Explicitly cast all values to real to prevent type mismatch
    RETURN QUERY SELECT
        p_title_id,
