-- Update the get_top_recommendations_v3 function to use Rotten Tomatoes scores for popularity/quality
...
BEGIN
    ...
    -- Base quality scores - USE ROTTEN TOMATOES when available, fallback to TMDB
    base_scores AS (
        SELECT 
            t.id AS tid,
            -- Quality: Prefer RT scores (combine critic + audience), fallback to TMDB vote_average
            CASE 
                WHEN t.rt_cscore IS NOT NULL AND t.rt_ascore IS NOT NULL THEN
                    -- Average of critic and audience scores, weighted toward audience (60% audience, 40% critic)
                    GREATEST(((COALESCE(t.rt_cscore, 0) * 0.4 + COALESCE(t.rt_ascore, 0) * 0.6) / 100.0), 0.5)::REAL
                WHEN t.rt_cscore IS NOT NULL THEN
                    GREATEST((COALESCE(t.rt_cscore, 0) / 100.0), 0.5)::REAL
                WHEN t.rt_ascore IS NOT NULL THEN
                    GREATEST((COALESCE(t.rt_ascore, 0) / 100.0), 0.5)::REAL
                ELSE
                    -- Fallback to TMDB vote_average
                    GREATEST((COALESCE(t.vote_average, 6) / 10.0), 0.5)::REAL
            END as quality_score,
            -- Popularity: Use RT audience count if available, otherwise TMDB popularity
            CASE
                WHEN t.rt_acount IS NOT NULL AND t.rt_acount > 0 THEN
                    -- Normalize RT audience count (log scale, max around 100k reviews = 1.0)
                    LEAST(LN(COALESCE(t.rt_acount::REAL, 1) + 1) / 11.5, 1.0)::REAL
                ELSE
                    -- Fallback to TMDB popularity (log scale)
                    LEAST(LN(COALESCE(t.popularity::REAL, 1) + 1) / 7.0, 1.0)::REAL
