...
-- Fix type mismatch: cast language_bonus and vibe_match_score to REAL
...
BEGIN
    ...
    SELECT 
        ...
        ((
            ...
            COALESCE(es.emotion_match_score, 0.5::REAL) * (v_intent_weight + v_transform_weight) +
            COALESCE(vs.vibe_match_score, 0.5::REAL) * 0.10 +
            COALESCE(ss.social_score, 0::REAL) * v_social_weight +
            COALESCE(hs.genre_affinity, 0.5::REAL) * v_historical_weight +
            (1.0::REAL - COALESCE(hs.genre_affinity, 0.5::REAL)) * v_novelty_weight +
            (bs.quality_score * 0.7 + bs.popularity_score * 0.3) * v_base_weight
        ) * bs.language_bonus)::REAL as out_final_score,
        COALESCE(es.emotion_match_score, 0.5::REAL)::REAL as out_emotional_component,
        COALESCE(vs.vibe_match_score, 0.5::REAL)::REAL as out_vibe_component,
        COALESCE(ss.social_score, 0::REAL)::REAL as out_social_component,
        COALESCE(hs.genre_affinity, 0.5::REAL)::REAL as out_historical_component,
        (1.0::REAL - COALESCE(hs.genre_affinity, 0.5::REAL))::REAL as out_novelty_component,
